...
def compile(folder, main):
    with zipfile.ZipFile(folder + "\\" + main, 'r') as sb3zip:
    ...
    path = pathlib.Path(pathlib.Path(folder) / "cache").resolve()
    path.mkdir(parents=True, exist_ok=True)
    with open(str(path / main), "w") as scrapefile:
